...
class Trainier:
    ...
    def init_model(self):
        ...
        self.G = Generator(    
            ...
            task=self.args.task,
            attn_layers=self.args.attn_layers).to(self.device)
    ...
    def train_one_epoch(self):
        ...
        for idx, batch_data in enumerate(self.train_loader):
            gli, token = batch_data['gli'].to('cuda'), batch_data['token'].to('cuda')
            ...
            w = self.G.get_w(gli, f_text=f_text)
            input_noise = self.G.make_image_noise(gli)
            y_hat = self.G(w, input_noise, f_text)
            
            # adversarial loss
            if self.args.adv_lambda > 0: 
                d_loss_dict = self.train_discriminator(real_img=gli, fake_img=y_hat)

            loss, loss_dict = self.calc_loss(y=gli, y_hat=y_hat)
    ...
    def validate(self):
        ...
        for idx, batch_data in enumerate(self.train_loader):
            ...
            with torch.no_grad():
                ...
                d_loss_dict = self.validate_discriminator(real_img=gli, fake_img=y_hat)
                loss, loss_dict = self.calc_loss(y=gli, y_hat=y_hat)
    ...
    def train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.max_epochs):
            ...
            run_g_loss, run_dice_loss, run_d_loss, run_lpips_loss = self.train_one_epoch()
            ...
            print(
                ...
                "lpips_loss: {:.4f}".format(run_lpips_loss),
            ...
            if (epoch + 1) % self.args.val_every == 0:
                ...
                epoch_time = time.time()
                val_g_loss, val_dice_loss, val_d_loss, val_dice = self.validate()

                print(
                    "Final validation stats {}/{}".format(epoch, self.args.max_epochs - 1),
                    ", val_g_loss | val_dice_loss | val_d_loss | val_dice:",
                    val_g_loss, val_dice_loss, val_d_loss, np.mean(val_dice, axis=0),
                    ", time {:.2f}s".format(time.time() - epoch_time),
                )
                ...
                if val_dice < G_loss_min:
                    print("new best ({:.6f} --> {:.6f}). ".format(G_loss_min, val_dice))
                    G_loss_min = val_dice
