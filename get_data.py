...
def get_data(path_list, dest, json_dest, phase='train'):    
    ...
    for i, path in enumerate(path_list):
        ...
        if key_idx == -1:
            ...
            idx_gli_image_save = []
            for j in range(tc_start, tc_end, max(3, (tc_end - tc_start) // 10)):
                idx_gli_image_save += [j]
            idx_gli_image_save = sorted(set(idx_gli_image_save))
            print("Slice index used for gli gen and image gen:", idx_gli_image_save)
            idx_used = idx_text_consis_save + idx_gli_image_save
            ...
            #             t2f=data['image'][idx][2], t2w=data['image'][idx][3])
                ...
                np.save(os.path.join(case_dest, "{}-{}.npy".format(t1c_name, idx)), data['image'][idx][0])   
                np.save(os.path.join(case_dest, "{}-{}.npy".format(t1n_name, idx)), data['image'][idx][1])  
                np.save(os.path.join(case_dest, "{}-{}.npy".format(t2f_name, idx)), data['image'][idx][2])   
                np.save(os.path.join(case_dest, "{}-{}.npy".format(t2w_name, idx)), data['image'][idx][3])    
                np.save(os.path.join(case_dest, "{}-{}.npy".format(seg_name, idx)), data['label'][idx])   
...
def gather_json():
    ...
    with open('./Brats2023/ag/gli_edit_train.json', 'r') as f:
        train = json.load(f)
        
    with open('./Brats2023/ag/gli_edit_val.json', 'r') as f:
        val = json.load(f)
        
    with open('./Brats2023/ag/gli_edit_test.json', 'r') as f:
        test = json.load(f)
    ...
    with open('./Brats2023/ag/gli_edit.json', 'w') as f:
        json.dump(data, f, indent=4)
        
    with open('./Brats2023/ag/gli_edit.json', 'r') as f:
        train = json.load(f)
